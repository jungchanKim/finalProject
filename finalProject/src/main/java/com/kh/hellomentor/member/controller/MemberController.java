...
public class MemberController {
    ...
    public String loginMember(
       ...
       String url = "";
       Member loginUser = mService.loginUser(m); //동일한 아이디를 비교하여 회원정보 가져옴
       
       
              if(loginUser != null){  //회원정보가 있다면!
            	  
        	  if (passwordEncoder.matches(userPwd, loginUser.getUserPwd())) {
           //userPwd: 로그인화면에서 적은 비밀번호//loginUser.getUserPwd: DB에 저장된 암호화된 비밀번호를 비교 
    ...
    public String insertMember(@Validated Member m, HttpSession session, Model model, BindingResult bindingResult,
        ...
        // 비밀번호 암호화
        String changePassword = passwordEncoder.encode(userPwd);
        m.setUserPwd(changePassword);
